/**
 * XSS test vectors vendored from the DOMPurify test suite (`test/fixtures/expect.mjs`),
 * maintained by cure53 — the authors of DOMPurify, which this sanitizer wraps.
 * Source: https://github.com/cure53/DOMPurify (Apache-2.0 OR MPL-2.0).
 *
 * Only the attacker-controlled `payload` strings are kept; DOMPurify's own
 * `expected` outputs are intentionally dropped because our sanitizer runs a
 * different config (custom hooks, HTML+MathML+SVG profiles, conditional
 * `<style>`). Pass/fail is decided by our own oracle, not by these fixtures.
 *
 * Regenerate: fetch expect.mjs and extract unique `payload` fields.
 */

export const CURE53_PAYLOADS: readonly string[] = [
  ...
  '<button command="show-modal" commandfor="dialog">Show Modal</button>',
  '<button popovertargetaction="show" popovertarget="popover">Show Popover</button>',
