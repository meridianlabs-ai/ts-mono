...
export function VirtualList<T>({
  ...
  const snapshotAtScrollTop = useCallback(
    (scrollTop: number): VirtualListStateSnapshot => {
  ...
  const buildSnapshot = useCallback(
    (el: HTMLElement | null) => snapshotAtScrollTop(el?.scrollTop ?? 0),
    [snapshotAtScrollTop]
  );
  ...
  const programmaticScroll = useCallback(
    (run: () => void, onDone?: () => void, persist = false) => {
      ...
      const release = () => {
        const userTookOver =
          interactionSequenceRef.current !== interactionAtStart;
  ...
  useEffect(() => {
    ...
    // A host that owns scroll persistence hands over a container offset; it
    // restores like a saved position, anchored on the row under that offset
    // (an offset past the estimate-sized total still names the last row).
    const snapshot =
      initialScrollOffset === undefined
        ? getRestoreSnapshot()
        : snapshotAtScrollTop(initialScrollOffset);
