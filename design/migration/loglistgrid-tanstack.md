...
- **Dead code pruned ✅** — deleted the now-unreferenced AG leftovers: `gridKeyboardNavigation`, `useApplyColumnVisibility`, `useGridColumnRefit` (+test), `samples-grid/RotatedHeader` (+css), `log-list/grid/PreformattedTooltip`, and the AST→AG-`FilterModel` conversion (`sample-tools/{astToFilterModel,filterModelToText,filterRegistry}` +tests); trimmed the dead `createGridColumnResizer` from `gridUtils`. (`colorScale` is kept — still used by `samples/header-v2/ScorePanel`; `gridComparators`/`gridUtils.getFieldKey`/`ColumnSelectorPopover`/`useSampleGridState` stay live but still carry AG *type* imports.)
- Cleanup follow-ups: (a) `SamplesGrid` imports the listing engine from `log-list/listing/` (a shared→feature direction) — the transitional `listing/` dir should move to `shared/` now that both grids use it; (b) the remaining AG *type*-only imports (the dormant `GridState` slice in `app/types.ts`/`logsSlice`/`state/hooks`, the `ColDef` picker shims, `IRowNode` in `gridComparators`) go away when grid-state persistence is rebuilt and the picker is de-AG'd.
...
- **TODO: next/prev sample arrows disabled on direct navigation to a sample.** Not a regression — main behaves the same way. Almost certainly a side effect of the sample-list hook never having been called on that path (the list the arrows navigate within was never populated). When fixing, follow the pattern scout used.
...
### Parity sweep findings — 2026-07-04, data/config plumbing (two live viewers: main@7576, branch dev@5173→view-server@7575)
...
No action (recorded so the next sweep doesn't re-flag them):

- Branch no longer polls `pending-samples` for completed logs (main fires it and 404s) — improvement.
...
- **Cold-dir listing paint**: the preview first wave (25) claims before the details backfill, and every details settle derives + persists its preview, so rows fill progressively — but on a large cold dir the preview *tail* (rows 26+) arrives at details pace, slower than pre-branch (where light previews had their own queue). Follow-up: demote synced missing-details backfill below the preview tail, or High-wave all cold previews.
- **`useLogDetail` demand is passive by default; only `useSelectedLogDetail` is active.** A passive mount (every sample-adjacent hook) ensures the data exists but never bumps `details_settled_seq` and never re-enqueues a server-cache-bypassing refresh on a cache hit — so switching to the Samples tab (mounting a passive consumer for the already-selected log) can no longer refire `LogLoadController` and wipe the user's scorer selection. Active demand for the selected log still bumps and still refreshes.
- **The settle-seq bump is now persisted (`sink.writeFetchStates`), not cache-only.** A recreated/late-mounted per-handle `useLogFetchState` entry reads the bumped seq from the db instead of racing a stale `readFetchStates` scan back to null. Known, accepted consequence: on reload, a persisted seq from a prior session can make `LogLoadController`'s effect fire once on mount (cached data) and once more after this session's settle — idempotent, same shape as selection-init already tolerates.
- **One unified Log row + depth — entity-model goal phase 3** (`log-data-summaries-entity.md`, phase-3 section). IndexedDB **schema v12** (one-time recreate): `log_previews`/`log_details`/`log_fetch_state` fold into the `logs` entity row (identity + `depth` + flat attribute columns + `header` + retrieval facts). Cache collapses to `["log_data","logs",logDir]` (ordered `Log[]`) + per-entity `["log_data","log",logDir,name]`; `useLogDetail` → `useLog` (contract unchanged); engine surface is `ensure(file, {depth, priority, fresh?, demand?})`. Behavior notes: an mtime-invalidated file now keeps its listing row while re-fetching (`resetDepth` — identity kept, content dropped; previously the row vanished until the listing re-activated); listing new-file discovery rides the same deferred row collection as content (may lag one deferred pass; was fresh while only content deferred); retried-dedup reads `row.status` (same values, previously the previews map); dead `requestPreview`/`ensurePreviews` engine APIs deleted (no production consumers).
- **Sample summaries are their own entity store — entity-model goal phase 2** (`log-data-summaries-entity.md`). IndexedDB **schema v11** (one-time recreate, cold cache, like v10): new `sample_summaries` table keyed `[file_path+id+epoch]`; `log_details` rows now store the header only plus ingestion-derived sample facts (`sampleCount`/`sampleErrorCount`/`sampleLimits`). The sink splits each details payload in one transaction; scoped reads (`useSamplesListing`: one file, or a path prefix) are db-backed, invalidation-driven, `keepPreviousData`. `useLogDetails` (whole-dir map) left the barrel; SamplesPanel/LogListGrid/columns read listing rows (`header` joined in) + `useSamplesListing` + `useScoreSchema`. Behavior notes: opening a large log no longer parses every summary on the read path (headers are summary-free); samples-mode rows refresh via invalidation refetch (Dexie read, batched per 250 ms flush) instead of render-path re-aggregation — content identical, timing shifts by up to one flush; `engine.fetch()` resolves `void` (no caller consumed the payload); the samples-mode grid's "raw mode" score discovery now scans scoped sample rows (was: embedded arrays), same result set.
- **Panels read the listing via one hook (`useLogListing(logDir): LogListingRow[]`) — entity-model goal phase 1** (`log-data-entity-model-goal.md`). `useLogHandlesWithRetried` and `useLogPreviews` left the public barrel; the handles ⋈ previews join and the `useDeferredValue` preview damping moved inside `log_data/logListing.ts`. Timing shift: retried-run dedup and SamplesPanel's completed-task count now read *deferred* previews like row content already did, so during a preview flush burst they can lag one deferred pass behind (previously dedup/count used fresh previews while row content deferred). Same settled result.
...
- **`SamplesGrid` feature restoration** — the grid is stood up on `DataGrid` with sorting + per-column filtering wired (see "Samples grid — stood up on DataGrid" above); what remains is the rest of the deferred list there (rotated headers, colour scales, follow-output, pinning/resize/reorder, grid-state persistence, Reset-Filters/filtered-count chrome).
- **`ScoreAgGrid` migration — the last AG Grid render.** `app/log-view/title-view/ScoreAgGrid.tsx` (the score-results grid in the log title view, used by `ResultsPanel` + `CollapsedTitleBar`) is the only remaining `<AgGridReact>`. It was never part of the collection-views scope. Migrating it to `DataGrid` (or replacing it) is what finally lets `ag-grid-community`/`ag-grid-react` + the `agGrid.ts` module registration be removed from the build, along with the residual AG *type* imports.
