...
(1) derive `loading`/error from query `AsyncData` (remove imperative `setLoading`
from data-load paths); (2) unify `activateReplication`/`syncLogs` into one owner;
(3) collapse the startup gate / delete `PushedQuerySource` (verify the
always-present-embedded assumption first).
...
Also deferred to a later "harden the boundaries" pass: formalizing layering as
eslint import-paths, and mechanically pushing all replication *status*
(`syncing`/`dbStats`) off zustand. This pass leans toward those (see rubric) but
...
- Structural invariants (greppable):
  ...
  - `loading`/error for data loads derive from `AsyncData`; no `setLoading(true/
    false)` bracketing IO in the data-load paths.
