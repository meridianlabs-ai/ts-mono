...
function collectFromContent(
  ...
  // Build a lookup from forkedAt UUID → branches for inline emission.
  // Branches whose forkedAt doesn't match any event are emitted at the end.
  const branchByForkedAt = new Map<string, TimelineSpan[]>();
  if (branches) {
    for (const branch of branches) {
      if (branch.forkedAt) {
        const existing = branchByForkedAt.get(branch.forkedAt);
        if (existing) {
          existing.push(branch);
        } else {
          branchByForkedAt.set(branch.forkedAt, [branch]);
        }
      }
    }
  }
  const emittedForkedAts = new Set<string>();
  ...
  // Emit any branches whose forkedAt didn't match any event in this content
...
function emitInlineBranches(
  item: TimelineEvent,
  branchByForkedAt: ReadonlyMap<string, TimelineSpan[]>,
  ...
  if (!uuid) return;
  const forkedBranches = branchByForkedAt.get(uuid);
 ...
 *
 * Branch keys follow the pattern `"parentKey/branch-{forkedAt}-{index}"`.
...
function buildAncestorChain(
  ...
  while (true) {
    ...
    if (!childSpan?.forkedAt) break;

    chain.unshift({ span: parentSpan, forkUuid: childSpan.forkedAt });
