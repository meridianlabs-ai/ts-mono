 ...
 * parent span (resolved via the branch's `forkedAt` UUID). Clicking a branch
 * marker toggles the showBranches display option.
 */
 ...
 *
 * Searches direct events in the parent for a UUID matching `branch.forkedAt`.
 * Falls back to `branch.startTime` when `forkedAt` is missing or not found.
 */
function resolveForkTimestamp(
  parent: TimelineSpan,
  branch: TimelineSpan
): Date {
  if (!branch.forkedAt) return branch.startTime;
  for (const item of parent.content) {
    if (item.type === "event" && item.event.uuid === branch.forkedAt) {
