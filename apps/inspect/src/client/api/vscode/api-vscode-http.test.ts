...
describe("apiVscodeHttp", () => {
  test("open_log_file posts a one-way displayLogFile message", async () => {
    ...
    await apiVscodeHttp(api, LOG_DIR).open_log_file("/logs/x.eval", "/logs");
  ...
  test("client_events is disabled (returns empty array)", async () => {
    const { api } = fakeVscode();
    await expect(apiVscodeHttp(api, LOG_DIR).client_events()).resolves.toEqual(
      []
    );
  });
  ...
  test("download_log is not exposed", () => {
    const { api } = fakeVscode();
    expect(apiVscodeHttp(api, LOG_DIR).download_log).toBeUndefined();
  });

  test("eval_log_sample_data_direct is not exposed", () => {
    const { api } = fakeVscode();
    expect(
      apiVscodeHttp(api, LOG_DIR).eval_log_sample_data_direct
    ).toBeUndefined();
  });
});
