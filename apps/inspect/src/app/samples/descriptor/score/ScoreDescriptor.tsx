...
export const getScoreDescriptorForValues = (
  uniqScoreValues: ScoreValueOrUnchanged[],
...
const scoreCategorizers: ScoreCategorizer[] = [
  {
    describe: (_values: ScoreValueOrUnchanged[], types?: ScorerTypes[]) => {
  ...
  {
    describe: (values: ScoreValueOrUnchanged[], _types?: ScorerTypes[]) => {
      if (
        values.length === 2 &&
        values.every((val) => {
          return val === 1 || val === 0;
  ...
  {
    describe: (values: ScoreValueOrUnchanged[], types?: ScorerTypes[]) => {
      if (
        types &&
        types[0] === "string" &&
        types.length === 1 &&
  ...
  {
    describe: (_values: ScoreValueOrUnchanged[], _types?: ScorerTypes[]) => {
