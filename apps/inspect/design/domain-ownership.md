...
cache entry `["app-config"]`. `api` and `logDir` are one snapshot: an api
instance is bound to one dir at construction (the `ClientAPI` contract), so a
dir change rebuilds the config — new instance, new dir, published together —
...
(user-initiated local-data reset). The _initialize_ verb is the
`<FetchEngineController>` mount at the composition root: it starts the engine
from the resolved config snapshot and restarts it when the config changes.
Growing the `ImperativeLogData` interface is a design decision,
...
| Local log database   | Persistence of Log entity rows + sample summaries (IndexedDB, per-dir). IndexedDB is required to activate directory mode; a blocked or unavailable database is surfaced as a storage-permission error rather than silently degrading to an empty cache. Single-file mode intentionally runs cache-only. The engine is the sole reader; every write goes through the sink. The service singleton creates lazily on first use (construction is side-effect free; opening is activation's job).                                                                                                                                                                                                                        | inside the engine; instance in `log_data/databaseServiceInstance.ts`                      |
...
`main.tsx` (the pre-gate bootstrap read — backend transport capabilities, no
api instance — and `initializeStore(capabilities, storage)`), plus the
`<FetchEngineController>` mount in `AppContent`: the engine's mode-aware
start/restart from the resolved config snapshot. **Roots are exempt from the
containment rules** — they construct interiors, so they may see constructors
and pre-resolution state. Nothing else is exempt.
...
App configuration         surface: useAppConfig / useLogDir / getAppConfig
                          interior: urlLogSource · resolveBackend · singleFileMode ·
